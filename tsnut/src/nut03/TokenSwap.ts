...
export class TokenSwap extends ITokenService {
    ...
    private async _swap(savedSwap: SavedTokenSwap): Promise<BlindSignature[]> {
        ...
        const inputProofsWithPoint = this.verifyInputProofsGetPoints(proofs);
        if(inputProofsWithPoint==null)
        ...
        if(await this.secretStorage.hasAnyOfBatchAndLock(inputProofsWithPoint, actionId)) {
        ...
        try {
            await this.secretStorage.addBatch(inputProofsWithPoint, actionId);
        } catch (e) {
            await this.secretStorage.unlockBatch(inputProofsWithPoint, actionId);
            await this.swapStorage.remove(actionId);
            throw e;
        }
    ...
    async start(): Promise<void> {
        ...
        for(let swap of savedSwaps) {
            ...
            if(swap.getState()===SavedTokenSwapState.LOCKING_INPUTS) {
                ...
                await this.secretStorage.unlockTryBatch(this.getPoints(swap.getRequest().inputs), actionId);
            }
            if(swap.getState()===SavedTokenSwapState.INPUTS_LOCKED) {
                ...
                await this.secretStorage.removeTryBatch(this.getPoints(swap.getRequest().inputs), actionId);
            }
