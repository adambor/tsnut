...
> extends IMintMeltTokenService<T> {
    ...
    async start(): Promise<void> {
        ...
        for(let savedMelt of savedMelts) {
            ...
            if(savedMelt.getState()===SavedTokenMeltState.LOCKING_INPUTS) {
                ...
                await this.secretStorage.unlockTryBatch(this.getPoints(savedMelt.inputs), actionId);
            }
            let proofsWithPoints: (Proof & {point: IPoint<any>})[];
            if(savedMelt.getState()===SavedTokenMeltState.INPUTS_LOCKED) {
                ...
                proofsWithPoints = this.getPoints(savedMelt.inputs);
                await this.intiatePayment(savedMeltResp, proofsWithPoints);
            }
            if(savedMelt.getState()===SavedTokenMeltState.PAYING) {
                //Subscribe to payment result
                if(proofsWithPoints==null) proofsWithPoints = this.getPoints(savedMelt.inputs);
                this.subscribeToPaymentAndFinish(savedMelt, proofsWithPoints);
            }
        }
    }
    ...
    protected async intiatePayment(savedMeltResp: {obj: T, lockId: string}, proofsWithPoints: (Proof & {point: IPoint<any>})[]) {
        ...
        try {
            ...
            await this.secretStorage.unlockBatch(proofsWithPoints, actionId);
    ...
    private async subscribeToPaymentAndFinish(savedMelt: T, proofsWithPoints: (Proof & {point: IPoint<any>})[]): Promise<Res> {
        const actionId = savedMelt.getId();
        ...
        if(resp.paid) {
            await this.secretStorage.addBatch(proofsWithPoints, actionId);
    ...
    protected async _melt(request: Req, savedMeltResp: {obj: T, lockId: string}): Promise<Res> {
        ...
        const proofsWithPoints = this.verifyInputProofsGetPoints(sortedInputs);
        if(proofsWithPoints==null) {
            await this.storage.unlock(actionId, lockId);
        ...
        if(await this.secretStorage.hasAnyOfBatchAndLock(proofsWithPoints, actionId)) {
            savedMelt.inputs = null;
            savedMelt.setState(SavedTokenMeltState.INIT);
            await this.storage.saveAndUnlock(savedMelt, lockId);
        ...
        //8. Initiate payment
        await this.intiatePayment(savedMeltResp, proofsWithPoints);
        console.log("TokenMelt: Payment initiated");

        //9. Wait for payment result
        const secret = await this.subscribeToPaymentAndFinish(savedMelt, proofsWithPoints);

        console.log("TokenMelt: Payment finished ", secret);
